...
router.put("/portfolio", checkAuth, async (req,res) => {
    try {
        ...
        const selectedAsset = userPortfolio.currentAssets.filter((asset) => asset.name == req.body.asset)
        
        if ((selectedAsset[0].value === 0 && req.body.order === "Remove") || 
        (req.body.order === "Remove" && ((selectedAsset[0].value - req.body.amount) < 0))) {
            return res.status(400).json({
                data: "",
